...
<head>    
    ...
    transition: all 0.3s ease;   
        ...
        .scrollable::-webkit-scrollbar-thumb {{ background: var(--primary); border-radius: 10px; }}  #modeSelection {{ position: fixed; inset: 0; background: inherit; display: flex; align-items: center; justify-content: center; z-index: 9999; padding: 20px; }}    
    ...
    @keyframes float {{ 0%, 100% {{ transform: translateY(0); }} 50% {{ transform: translateY(-8px); }} }}    .mode-cards {{ display: grid; gap: 16px; margin-bottom: 5px; }}    
...
.question-section {{  
position: relative;  
padding: 8px;        /* 🔥 20px → 8px (side gap खत्म) */  
margin-top: 70px;    /* 🔥 थोड़ा और tight */  
margin-bottom: 90px; /* थोड़ा कम */

}}
.question-card {{
background: var(--bg-white);
border-radius: 18px;   /* थोड़ा कम किया (optional) /
padding: 20px;         / 28px → 20px */
box-shadow: var(--shadow-md);

max-width: 100%;       /* 🔥 FULL WIDTH */  
margin: 0;             /* 🔥 SIDE GAP REMOVE */  
...
.question-text {{ font-size: 28px; font-weight: 600; color: var(--text-dark); line-height: 1.8; margin-bottom: 12px; white-space: pre-wrap; word-wrap: break-word; }}
.option-btn {{ width: 100%; padding: 18px 20px; background: var(--bg-light); border: 3px solid var(--border); border-radius: 16px; text-align: left; font-size: 22px; color: var(--text-dark); cursor: pointer; display: flex; align-items: flex-start; gap: 14px; transition: 0.3s; margin-bottom: 10px; line-height: 1.6; }}
...
<body>    
...
</div>    <!-- 🔥 Telegram Join Button -->  <div style="text-align:center;">  
...
</div>  <div style="text-align:center; padding:20px; color:var(--text-light);">  
    Made with ❤️ by <b>{created_by}</b> </div>  <script>    
    ...
    <div class="question-card">
        <div class="question-text">
            <span class="q-no">Q${state.current + 1}.</span> ${q.text || ""}
        </div>
    `;

    // Image
    if (q.img) {
        h += `<img src="${q.img}" onerror="this.style.display='none'">`;
    }
    ...
    q.opts.forEach((o, i) => {
        ...
        const selected = state.ans[state.current] === i;
        const review = (state.mode === 'practice' && state.ans[state.current] !== null) || state.isSub;
    ...
    if ((state.mode === 'practice' && state.ans[state.current] !== null) || state.isSub) {{
        ...
        <div class="explanation-box" style="display:block;">
            <div style="font-weight:800; color:#92400E; margin-bottom:10px;">
                <i class="fas fa-lightbulb"></i> Explanation
            </div>
            <div style="font-size:22px; line-height:1.7; color:green;">
                ${q.sol || "No explanation"}
            </div>
        </div>
        `;
    }}
