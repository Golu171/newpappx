...
#modeSelection {{
    ...
    .question-text {{ font-size: 28px; font-weight: 800; color: #8B0000; line-height: 1.45; margin-bottom: 12px; white-space: pre-wrap; word-wrap: break-word; }}  
    .option-btn {{ width: 100%; padding: 12px 14px; background: var(--bg-light); border: 3px solid var(--border); border-radius: 14px; text-align: left; font-size: 24px; color: #1565C0; font-weight: 800; cursor: pointer; display: flex; align-items: flex-start; gap: 14px; transition: 0.3s; margin-bottom: 10px; line-height: 1.6; }}  
...
<script>  
    ...
    function fitQuestionToScreen() {{
        ...
        const card = area ? area.querySelector('.question-card') : null;
        if (!area || !card) return;

        document.documentElement.style.setProperty('--fit-scale', '1');
        const available = Math.max(1, area.clientHeight - 12);
        const needed = Math.max(1, card.scrollHeight);

        let fit = Math.min(1, available / needed);
        // Keep text readable; only shrink when the complete question card cannot fit.
        fit = Math.max(0.65, fit);
        document.documentElement.style.setProperty('--fit-scale', fit);
    }}
    ...
    function renderQ() {{  
        ...
        q.opts.forEach((o, i) => {{  
            ...
            h += `<button class="${{cls}}" onclick="selectOpt(${{i}})">  
                <div class="option-indicator">${{String.fromCharCode(65+i)}}</div><div style="flex:1; color:#1565C0; font-weight:800;">${{o}}</div>  
            </button>`;  
        }});  
        h += `</div>`;  
