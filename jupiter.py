...
/* 🔥 thoda aur tight */
    ...
    .option-btn {{ width: 100%; padding: 18px 20px; background: var(--bg-light); border: 3px solid var(--border); border-radius: 16px; text-align: left; font-size: 24px; color: #000; cursor: pointer; display: flex; align-items: flex-start; gap: 14px; transition: 0.3s; margin-bottom: 10px; line-height: 1.6; }}  
    ...
    function renderQ() {{  
        ...
        if(((state.mode === 'practice' && state.ans[state.current] !== null) || state.isSub)) {{  
            h += `<div class="explanation-box" style="display:block;">  
                ...
                <div style="font-size:24px; line-height:1.7; color:#065F46;">${{q.sol}}</div>  
            </div>`;  
        }}  
        h += `</div>`;  
