...
:root{
 ...
 --border:#bbf7d0;
 --primary:#22c55e;
 --primary-glow:rgba(34,197,94,.25);
 --success:#2563eb;
 --success-bg:#dbeafe;
 --danger:#ef4444;
 --danger-bg:#fecaca;
...
.mode-container{
 background:var(--green-panel);border:4px solid #86efac;
 border-radius:28px;padding:30px 24px;max-width:520px;width:100%;
...
.mode-header h2{font-size:30px;line-height:1.25;font-weight:800;color:#000;margin-bottom:12px}
.mode-header p{color:#000;font-size:18px}
.mode-cards{display:grid;gap:16px;margin-bottom:18px}
.mode-card{
 background:var(--green-panel);border:3px solid var(--green-border);
 border-radius:18px;padding:18px;cursor:pointer;transition:.3s;display:flex;
 align-items:center;gap:15px;text-align:left
}
.mode-card.selected{border-color:#15803d;background:#bbf7d0;transform:translateY(-2px)}
.mode-icon{
...
.mode-card h3,.mode-card p{color:#000}

#customTimer{
 width:100%;padding:16px;border:3px solid #22c55e;border-radius:14px;
 margin-bottom:18px;font-size:18px;background:#dcfce7;color:#000
}
...
.quiz-header{
 position:fixed;top:0;left:0;right:0;background:var(--green-panel);
...
.zoom-btn{
 width:40px;height:40px;border-radius:12px;border:2px solid #86efac;
 background:#dcfce7;color:#000;cursor:pointer;font-size:18px;font-weight:800
}
.timer-display{
 background:#dcfce7;border:2px solid #86efac;padding:5px 10px;border-radius:12px;
 font-weight:800;color:#15803d;display:flex;align-items:center;gap:7px;font-size:18px
}
.header-info{
...
.progress-bar{height:100%;background:linear-gradient(90deg,#22c55e,#16a34a);transition:width .4s ease}

.question-section{position:relative;padding:8px;margin:0;overflow:visible}
.question-card{
 background:var(--green-panel);border:4px solid #86efac;border-radius:24px;
...
.question-number{
 ...
 color:#1d4ed8;background:#dbeafe;padding:8px 16px;border-radius:25px;margin-bottom:20px
}
.question-text{
...
.option-indicator{
 min-width:38px;height:38px;border-radius:50%;background:#dcfce7;
...
.option-btn.selected{
 background:#bbf7d0;border-color:#15803d
}

/* CORRECT = BLUE */
.option-btn.correct{
 background:#bfdbfe;border-color:#2563eb;color:#000!important
}
...
.option-btn.incorrect{
 background:#fecaca;border-color:#ef4444;color:#000!important
}
.option-btn.incorrect .option-indicator{
 background:#ef4444;color:#fff!important;border:none
}
.option-btn.disabled{cursor:default}

.explanation-box{
 display:none;background:var(--green-panel);border:3px solid #86efac;
 border-left:6px solid #22c55e;border-radius:0 18px 18px 0;
 padding:18px;margin-top:22px
}
.explanation-title{font-weight:800;color:#000!important;margin-bottom:10px;font-size:24px}
.explanation-text{font-size:22px;line-height:1.65;color:#000!important;font-weight:700}
.explanation-text *{color:#000!important}

.nav-controls{
 position:fixed;bottom:0;left:0;right:0;background:var(--green-panel);
 padding:14px 18px;box-shadow:0 -4px 20px rgba(0,0,0,.08);
 display:flex;gap:12px;z-index:90;border-top:2px solid #86efac
}
.question-nav-toggle{
...
.question-nav-panel{
 ...
 overflow-y:auto;background:var(--green-panel);border-top:3px solid #86efac;
 border-radius:28px 28px 0 0;box-shadow:0 -8px 40px rgba(0,0,0,.2);z-index:95;
...
.question-nav-item{
 aspect-ratio:1;border:3px solid #22c55e;border-radius:14px;
 display:flex;align-items:center;justify-content:center;font-weight:800;
 cursor:pointer;color:#000;background:#bbf7d0
}
.question-nav-item.current{border-color:#2563eb;color:#1d4ed8;background:#dbeafe}
.question-nav-item.answered{background:#bbf7d0;color:#000}
.question-nav-item.marked{background:#f59e0b;color:#000;border:none}
.question-nav-item.correct{background:#2563eb;color:#fff;border:none}
...
/* RESULTS */
#resultsContainer{
 display:none;position:fixed;inset:0;background:var(--green-bg);overflow-y:auto;
 padding:20px;z-index:1000;text-align:center
}
.results-card{
 background:var(--green-panel);border:4px solid #86efac;border-radius:28px;
 padding:40px 30px;max-width:600px;margin:0 auto 20px
}
.results-score{font-size:56px;font-weight:800;color:#2563eb}
.stats-grid{
 display:grid;grid-template-columns:1fr 1fr;gap:14px;max-width:600px;margin:0 auto 24px
}
.stat-card{background:var(--green-panel);border:3px solid #86efac;padding:24px;border-radius:20px}
 ...
 <div class="quiz-header">
  <div class="header-top">
   ...
   <div class="header-actions">
    ...
    <!-- DARK THEME / MOON BUTTON REMOVED -->
...
<div id="resultsContainer">
 ...
 <div class="results-card">
  ...
  <div id="resPercent" style="font-size:22px;font-weight:600;color:#000">0%</div>
 </div>

 <div class="stats-grid">
  <div class="stat-card">
   <div class="stat-val" style="color:#2563eb" id="sCorrect">0</div>
