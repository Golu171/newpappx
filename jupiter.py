...
.exam-mode .mode-icon {{ background: #EF4444; }} .practice-mode .mode-icon {{ background: #10B981; }}      #quizContainer {{
display: none;
position: fixed;
inset: 0;
...
<body>      
    ...
    const state = {{ current: 0, qs: [], ans: [], marked: [], mode: null, time: 3600, isSub: false, theme: 'light', zoom: 1 }};      function setMode(m, el) {{      
    state.mode = m;      
...
function renderQ() {{      
    ...
    if(((state.mode === 'practice' && state.ans[state.current] !== null) || state.isSub)) {{      
        h += `<div class="explanation-box" style="display:block;">      
            ...
            <div style="font-size:22px; line-height:1.7; color:green;">${{q.sol}}</div>      
        </div>`;      
    }}      
    h += `</div>`;      
