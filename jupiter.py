...
#modeSelection {{
    ...
    .mode-container {{ position: relative; background: rgba(255,255,255,0.95); backdrop-filter: blur(20px); border-radius: 28px; padding: 40px 30px; max-width: 520px; width: 100%; box-shadow: var(--shadow-lg); text-align: center; }}  
    ...
    transform: scale(calc(var(--content-zoom) * var(--fit-scale)));
    ...
    .question-text {{ font-size: 28px; font-weight: 800; color: #000000; line-height: 1.45; margin-bottom: 12px; white-space: pre-wrap; word-wrap: break-word; }}  
    .option-btn {{ width: 100%; padding: 12px 14px; background: var(--bg-light); border: 3px solid var(--border); border-radius: 14px; text-align: left; font-size: 24px; color: #000000; font-weight: 800; cursor: pointer; display: flex; align-items: flex-start; gap: 14px; transition: 0.3s; margin-bottom: 10px; line-height: 1.6; }}  
    ...
    #resultsContainer {{ display: none; position: fixed; inset: 0; background: #DDF5E3; overflow-y: auto; padding: 20px; z-index: 1000; text-align: center; }}  
    ...
    .results-score {{ font-size: 56px; font-weight: 800; background: #DDF5E3; -webkit-background-clip: text; -webkit-text-fill-color: transparent; }}  
    ...
    [data-theme="dark"] .option-btn > div:last-child {{
        color: #000000 !important;
    }}
    ...
    .telegram-start {{
        ...
        animation: telegramMove 1.8s ease-in-out infinite, telegramPulse 1.8s ease-in-out infinite;
        z-index: 10;
    }}
    ...
    @keyframes telegramPulse {{
        0%, 100% {{ box-shadow: 0 6px 20px rgba(34,158,217,0.40); }}
        50% {{ box-shadow: 0 9px 28px rgba(34,158,217,0.65); }}
    }}
    ...
    <div id="modeSelection">
        <div class="mode-container">
            <a href="https://t.me/MOCK_TEST18" target="_blank" rel="noopener noreferrer"
               class="telegram-start" aria-label="Join Telegram Channel">
...
<script>  
    ...
    function fitQuestionToScreen() {{
        // Keep the question area scrollable so the explanation remains accessible.
    ...
    function renderQ() {{  
        ...
        q.opts.forEach((o, i) => {{  
            ...
            h += `<button class="${{cls}}" onclick="selectOpt(${{i}})">  
                <div class="option-indicator">${{String.fromCharCode(65+i)}}</div><div style="flex:1; color:#000000; font-weight:800;">${{o}}</div>  
            </button>`;  
        }});  
        h += `</div>`;  
    ...
    window.addEventListener('resize', () => requestAnimationFrame(fitQuestionToScreen));
    window.addEventListener('orientationchange', () => setTimeout(fitQuestionToScreen, 150));
