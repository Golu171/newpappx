...
#modeSelection {{
    ...
    .mode-container {{ position: relative; background: rgba(255,255,255,0.95); backdrop-filter: blur(20px); border-radius: 28px; padding: 78px 30px 40px; max-width: 520px; width: 100%; box-shadow: var(--shadow-lg); text-align: center; }}  
    ...
    .question-text {{ font-size: 22px; font-weight: 800; color: #F97316; line-height: 1.45; margin-bottom: 12px; white-space: pre-wrap; word-wrap: break-word; }}  
    .option-btn {{ width: 100%; padding: 12px 14px; background: var(--bg-light); border: 3px solid var(--border); border-radius: 14px; text-align: left; font-size: 22px; color: #15803D; font-weight: 800; cursor: pointer; display: flex; align-items: flex-start; gap: 14px; transition: 0.3s; margin-bottom: 10px; line-height: 1.6; }}  
    ...
    .explanation-box {{ display: none; background: transparent; border-left: 5px solid #16A34A; border-radius: 0 16px 16px 0; padding: 20px; margin-top: 24px; animation: slideDown 0.4s ease; }}  
    ...
    function renderQ() {{  
        ...
        q.opts.forEach((o, i) => {{  
            ...
            h += `<button class="${{cls}}" onclick="selectOpt(${{i}})">  
                <div class="option-indicator">${{String.fromCharCode(65+i)}}</div><div style="flex:1; color:#15803D; font-weight:800;">${{o}}</div>  
            </button>`;  
        }});  
        h += `</div>`;  
        if(((state.mode === 'practice' && state.ans[state.current] !== null) || state.isSub)) {{  
            h += `<div class="explanation-box" style="display:block;">  
                <div style="font-weight:800; color:#16A34A !important; margin-bottom:10px;"><i class="fas fa-lightbulb"></i> Explanation</div>  
                <div style="font-size:28px; line-height:1.7; color:#15803D !important; font-weight:700;">${{q.sol}}</div>  
            </div>`;  
        }}  
        h += `</div>`;  
