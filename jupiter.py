...
#modeSelection {{
    ...
    .mode-cards {{ display: grid; gap: 16px; margin-bottom: 5px; }}  
    ...
    .quiz-header {{ position: fixed; top: 0; left: 0; right: 0; background: var(--bg-white); box-shadow: var(--shadow-sm); z-index: 100; padding: 6px 10px; }}  
    .header-top {{ display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; }}  
    .timer-display {{ background: var(--primary-glow); padding: 5px 10px; border-radius: 12px; font-weight: 700; color: var(--primary); display: flex; align-items: center; gap: 8px; }}  
...
/* 🔥 thoda aur tight */
    margin-bottom: 90px; /* थोड़ा कम */
}}  
    .question-card {{ 
    background: var(--bg-white); 
    border-radius: 18px;   /* à¤¥à¥‹à¤¡à¤¼à¤¾ à¤•à¤® à¤•à¤¿à¤¯à¤¾ (optional) */
    padding: 20px;         /* 28px â†’ 20px */
    box-shadow: var(--shadow-md); 
    
    max-width: 100%;       /* ðŸ”¥ FULL WIDTH */
    margin: 0;             /* ðŸ”¥ SIDE GAP REMOVE */
    ...
    .question-text {{ font-size: 28px; font-weight: 600; color: #065F46; line-height: 1.8; margin-bottom: 12px; white-space: pre-wrap; word-wrap: break-word; }}  
    .option-btn {{ width: 100%; padding: 18px 20px; background: var(--bg-light); border: 3px solid var(--border); border-radius: 16px; text-align: left; font-size: 24px; color: #065F46;font-weight: 700; cursor: pointer; display: flex; align-items: flex-start; gap: 14px; transition: 0.3s; margin-bottom: 10px; line-height: 1.6; }}  
...
<body>  
    ...
    <div id="hanumanWelcome" style="position:fixed; inset:0; z-index:10000; display:flex; align-items:center; justify-content:center; background:linear-gradient(135deg,#fff7ed,#fed7aa,#ffedd5); padding:20px; text-align:center;">
        <div style="max-width:520px; width:100%; position:relative; background:rgba(255,255,255,0.96); border-radius:28px; padding:28px 22px; box-shadow:0 10px 40px rgba(0,0,0,0.18);">
            <a href="https://t.me/MOCK_TEST18" target="_blank" rel="noopener noreferrer"
               aria-label="Join Telegram Channel"
               style="position:absolute; top:12px; right:12px; width:34px; height:34px; border-radius:50%; display:flex; align-items:center; justify-content:center; background:#229ED9; color:white; text-decoration:none; font-size:17px; box-shadow:0 4px 10px rgba(0,0,0,0.18);">
                <i class="fab fa-telegram-plane"></i>
            </a>

            <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/5/5f/Hanuman_in_Panchmukhi_form.jpg/640px-Hanuman_in_Panchmukhi_form.jpg"
                 alt="श्री हनुमान जी"
                 style="width:210px; height:210px; object-fit:cover; border-radius:22px; margin:0 auto 18px; display:block; border:2px solid #f59e0b;"
                 onerror="this.style.display='none';">
            <div style="font-size:32px; font-weight:800; color:#b45309; margin-bottom:8px;">🚩 जय बजरंग बली 🚩</div>
            <div style="font-size:17px; font-weight:600; color:#92400e; margin-bottom:22px;">श्री हनुमान जी का आशीर्वाद</div>
            <button onclick="enterQuiz()" style="border:none; border-radius:14px; padding:14px 30px; font-size:18px; font-weight:700; color:white; background:linear-gradient(135deg,#f97316,#ea580c); cursor:pointer; box-shadow:0 6px 18px rgba(234,88,12,0.3);">
                🚩 शुरू करें
            </button>
        </div>
    </div>
    ...
    <div class="quiz-header">  
        <div class="header-top">  
            ...
            <div class="header-actions" style="display:flex; gap:8px; align-items:center;">  
                <!-- 🔥 Zoom Controls Added Next to Moon Button -->
                <button onclick="zoomOut()" style="width:36px; height:36px; border-radius:10px; border:2px solid var(--border); background:var(--bg-light); cursor:pointer;"><i class="fas fa-minus"></i></button>
                <button onclick="zoomIn()" style="width:36px; height:36px; border-radius:10px; border:2px solid var(--border); background:var(--bg-light); cursor:pointer;"><i class="fas fa-plus"></i></button>
...
<div style="text-align:center;">
    <a href="https://t.me/MOCK_TEST18" target="_blank" 
       style="display:inline-block; margin:15px auto; padding:18px 25px; 
...
<div style="text-align:center; padding:20px; color:var(--text-light);">
    Made with ❤️ by <b>{created_by}</b> </div>
 

<script>  
    const rawData = {json_str};  
    const state = {{ current: 0, qs: [], ans: [], marked: [], mode: null, time: 3600, isSub: false, theme: 'light', zoom: 1 }};  


    function enterQuiz() {
        const welcome = document.getElementById('hanumanWelcome');
    ...
    function renderQ() {{  
        ...
        q.opts.forEach((o, i) => {{  
            ...
            h += `<button class="${{cls}}" onclick="selectOpt(${{i}})">  
                <div class="option-indicator">${{String.fromCharCode(65+i)}}</div><div style="flex:1; color:#065F46;">${{o}}</div>  
            </button>`;  
        }});  
        h += `</div>`;  
        if(((state.mode === 'practice' && state.ans[state.current] !== null) || state.isSub)) {{  
            h += `<div class="explanation-box" style="display:block;">  
                ...
                <div style="font-size:28px; line-height:1.7; color:#065F46; font-weight:700;">${{q.sol}}</div>  
            </div>`;  
        }}  
        h += `</div>`;  
