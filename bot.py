...
async def explore_recursively(api_url, course_id, parent_id, tests_list, current_path="Main", headers=None):
...
async def get_api_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "akamai" in api_url:
        if "eduguru" in api_url or "gurukul" in api_url:
            context.user_data['headers'] = AGG_HEADERS
        else:
            context.user_data['headers'] = HSSC_HEADERS
    else:
...
async def handle_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    headers = context.user_data.get('headers', HEADERS)
    ...
    try:
        if choice == "course":
            p = {
                "search_term": "TEST SERIES",
                "user_id": "-1",
                "screen_name": "Dashboard"
            }

            r = scraper.post(
                f"{api_url}/get/search",
                headers=headers,
                json=p,
                timeout=15
            ).json()
...
async def start_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    headers = context.user_data.get('headers', HEADERS)   # 👈 ये add कर
