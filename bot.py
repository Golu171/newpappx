...
async def get_creator_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('is_akamai'):
        kb = [[InlineKeyboardButton("🔥 HSSC Special", callback_data="type_hssc")],
              [InlineKeyboardButton("📂 Aggregator (EduGuru)", callback_data="type_agg")]]
    else:
...
async def handle_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "hssc":
        await query.edit_message_text("🔢 *HSSC Course ID Bhejo:*")
        return HSSC_ID
    
    elif choice == "agg":
        await query.edit_message_text("📡 *Fetching Exam Sections... Wait bhai*")
        try:
            ...
            if resp.status_code != 200:
                await query.message.reply_text(f"❌ API Error: {resp.status_code}. Token check kar!")
            ...
            if not sections:
                await query.message.reply_text("❌ No sections found. API empty hai!")
                return ConversationHandler.END
            ...
            await query.message.reply_text(f"💥 Aggregator Error: {str(e)}")
            return ConversationHandler.END
...
async def get_hssc_course_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        context.user_data['item_names'] = {str(i[0]): i[1] for i in items}
        btns = [[InlineKeyboardButton(i[1][:40], callback_data=f"sel_{i[0]}")] for i in items[:40]]
        await update.message.reply_text("🎯 *Select Folder:*", reply_markup=InlineKeyboardMarkup(btns))
        return SELECT_ITEM
    except Exception as e:
        await update.message.reply_text(f"❌ ID Fetch Error: {e}")
        return ConversationHandler.END
...
async def section_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        r = scraper.get(url, headers=headers, timeout=15).json()
        items = [(ts["id"], ts["title"]) for ts in r.get("data", [])]
        context.user_data['item_names'] = {str(i[0]): i[1] for i in items}
        btns = [[InlineKeyboardButton(i[1][:40], callback_data=f"sel_{i[0]}")] for i in items[:40]]
        ...
        await query.message.reply_text(f"❌ Series Error: {e}")
        return ConversationHandler.END
...
async def item_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await query.edit_message_text("🕵️ *Deep Scanning Content...*")
    all_tests = []
    ...
    try:
        ...
        await query.message.reply_text(f"❌ Item Scan Error: {e}")
        return ConversationHandler.END
...
async def start_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await query.edit_message_text(f"⚡ Processing `0/{len(all_tests)}`...")
