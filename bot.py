...
async def get_api_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "hsscguide" in url and "api" not in url:
        url = url.replace("hsscguide.", "hsscguideapi.")
    ...
    if "akamai" in api_url:
        ...
        await update.message.reply_text("🛡️ *Akamai Detected!*\n✍️ Creator Name?")
    else:
...
async def get_creator_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('is_akamai'):
        kb = [
            [InlineKeyboardButton("🚀 HSSC Special (Auto)", callback_data="type_hssc")],
...
async def handle_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "hssc":
        await query.edit_message_text("📡 *Fetching HSSC Series/Packages... Wait bhai*")
        try:
            # HSSC ke liye search endpoint best hai list nikalne ke liye
            p = {"search_term": "", "user_id": "22997", "screen_name": "Dashboard"}
            r = scraper.post(f"{api_url}/get/search", headers=headers, json=p, timeout=20).json()
            items = [(c["id"], c["course_name"]) for c in r.get("courses_with_folder", [])]
            
            if not items:
                r = scraper.get(f"{api_url}/get/test_series?start=-1", headers=headers, timeout=20).json()
            ...
            await query.message.reply_text("🎯 *Select Target:*", reply_markup=InlineKeyboardMarkup(btns))
            return SELECT_ITEM
        except Exception as e:
            await query.message.reply_text(f"💥 HSSC API Error: {str(e)}"); return ConversationHandler.END

    elif choice == "educourse":
        await query.edit_message_text("📡 *Fetching EduGuru Courses...*")
        try:
            base_api = "https://hsscgurukulapi.akamai.net.in"
            p = {"search_term": "", "user_id": "1558", "screen_name": "Dashboard"}
            r = scraper.post(f"{base_api}/get/search", headers=headers, json=p, timeout=20).json()
            items = [(c["id"], c["course_name"]) for c in r.get("courses_with_folder", [])]
            context.user_data['item_names'] = {str(i[0]): i[1] for i in items}
            btns = [[InlineKeyboardButton(i[1][:40], callback_data=f"sel_{i[0]}")] for i in items[:40]]
            await query.message.reply_text("🎯 *Select Batch:*", reply_markup=InlineKeyboardMarkup(btns))
            return SELECT_ITEM
        except: return ConversationHandler.END

    elif choice == "agg":
        await query.edit_message_text("📡 *Fetching Sections...*")
        try:
            agg_api, client_api = "https://thetestpassapi.akamai.net.in", "https://hsscgurukulapi.akamai.net.in/"
            url = f"{agg_api}/get/get_testseries_exams?start=0&search&client_api_url={client_api}"
            r = scraper.get(url, headers=headers).json()
            sections = [(item.get("exam_id") or item.get("id"), item.get("exam_name") or item.get("title")) for item in r.get("data", [])]
            btns = []
            for i in range(0, len(sections), 2):
                ...
                btns.append(row)
            await query.message.reply_text("📚 *Section Select Kar:*", reply_markup=InlineKeyboardMarkup(btns))
    ...
    try:
        ...
        context.user_data['item_names'] = {str(i[0]): i[1] for i in items}
        btns = [[InlineKeyboardButton(i[1][:40], callback_data=f"sel_{i[0]}")] for i in items[:40]]
        await query.message.reply_text("🎯 *Select Item:*", reply_markup=InlineKeyboardMarkup(btns))
        return SELECT_ITEM
    except: return ConversationHandler.END
...
async def section_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        items = [(ts.get("id") or ts.get("testseries_id"), ts.get("title") or ts.get("testseries_name")) for ts in data_list]
        context.user_data['item_names'] = {str(i[0]): i[1] for i in items}
        btns = [[InlineKeyboardButton(i[1][:40], callback_data=f"sel_{i[0]}")] for i in items[:40]]
        await query.message.reply_text("🎯 *Select Series:*", reply_markup=InlineKeyboardMarkup(btns))
        return SELECT_ITEM
    except: return ConversationHandler.END
...
async def item_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await query.edit_message_text("🕵️ *Deep Scanning Topic-Wise... Wait bhai.*")
    all_tests = []
    try:
        if mode == "agg":
            agg_api, client_url = "https://thetestpassapi.akamai.net.in", "https://hsscgurukulapi.akamai.net.in/"
            sub_resp = scraper.get(f"{agg_api}/get/testseries_subjects?testseries_id={item_id}&client_api_url={client_url}", headers=headers, timeout=15).json()
            for s in sub_resp.get("data", []):
                t_resp = scraper.get(f"{agg_api}/get/test_titlev2?testseriesid={item_id}&subject_id={s['subjectid']}&userid=1558&start=-1&client_api_url={client_url}", headers=headers, timeout=15).json()
        ...
        elif mode == "hssc" or "hsscguide" in context.user_data['api_url']:
            # HSSC Guide/Special: Try Recursive Scan first because these are usually Packages/Courses
            api_url = context.user_data['api_url']
            await explore_recursively(api_url, item_id, -1, all_tests, headers=headers)
            
            # If recursive scan found nothing, fallback to subject-based series scan
            if not all_tests:
                subj_url = f"{api_url}/get/testseries_subjects?testseries_id={item_id}"
                subj = scraper.get(subj_url, headers=headers, timeout=15).json()
            ...
            api_url = "https://hsscgurukulapi.akamai.net.in"
            await explore_recursively(api_url, item_id, -1, all_tests, headers=headers)
...
async def start_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, t in enumerate(all_tests):
        try:
            ...
            if i % 10 == 0: await status_msg.edit_text(f"⏳ Downloading: `{i}/{len(all_tests)}`...")
        except: continue
    if choice == "zip":
        ...
        await query.message.reply_document(document=open(zip_name, 'rb'), caption=f"🚀 Extraction Done!")
        os.remove(zip_name)
    else:
        for t in extracted:
            try:
                await query.message.reply_document(document=open(t['path'], 'rb'), caption=f"📁 {t['folder']}\n✅ {t['title']}")
