...
async def handle_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "hssc":
        ...
        try:
            ...
            # Key Fix: Handling both 'exam_id' and 'id'
            sections = []
            for item in r.get("data", []):
                s_id = item.get("exam_id") or item.get("id")
                s_name = item.get("exam_name") or item.get("title")
                if s_id and s_name:
                    sections.append((s_id, s_name))

            if not sections:
                await query.message.reply_text("❌ No sections found in API response.")
...
async def get_hssc_course_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"❌ ID Fetch Error: {e}"); return ConversationHandler.END
...
async def section_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    headers = context.user_data['mode_headers']
    try:
        # Fixed: Aggregator section selection hit the base API
