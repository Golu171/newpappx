...
async def extract_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("ðŸ”— *API URL Bhejo bhai:*")
    return API_URL
...
async def get_creator_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('is_akamai'):
        kb = [
            [InlineKeyboardButton("ðŸš€ HSSC Auto-List", callback_data="type_hssc")],
            [InlineKeyboardButton("ðŸŽ“ EduGuru Courses", callback_data="type_educourse")],
            [InlineKeyboardButton("ðŸ“‚ EduGuru Aggregator", callback_data="type_agg")]
        ]
    else:
    ...
    await update.message.reply_text("ðŸ¤” *Select Extraction Mode:*", reply_markup=InlineKeyboardMarkup(kb))
    return CHOOSE_TYPE
...
async def handle_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "educourse":
        await query.edit_message_text("ðŸ“¡ *Fetching EduGuru Courses... Wait bhai*")
        try:
            ...
            await query.message.reply_text("ðŸŽ¯ *Select EduGuru Course:*", reply_markup=InlineKeyboardMarkup(btns))
            return SELECT_ITEM
        except Exception as e:
            await query.message.reply_text(f"ðŸ’¥ Course API Error: {str(e)}"); return ConversationHandler.END

    elif choice == "hssc":
        await query.edit_message_text("ðŸ“¡ *Fetching HSSC Guide Series... Auto-Fetch*")
        try:
            r = scraper.get(f"{api_url}/get/test_series?start=-1", headers=headers, timeout=20).json()
            items = [(ts["id"], ts["title"]) for ts in r.get("data", [])]
            context.user_data['item_names'] = {str(i[0]): i[1] for i in items}
            btns = [[InlineKeyboardButton(i[1][:40], callback_data=f"sel_{i[0]}")] for i in items[:40]]
            await query.message.reply_text("ðŸŽ¯ *Select HSSC Series:*", reply_markup=InlineKeyboardMarkup(btns))
            return SELECT_ITEM
        except Exception as e:
            await query.message.reply_text(f"ðŸ’¥ HSSC API Error: {str(e)}"); return ConversationHandler.END

    elif choice == "agg":
        await query.edit_message_text("ðŸ“¡ *Fetching Aggregator Sections...*")
        try:
    ...
    try:
        ...
        context.user_data['item_names'] = {str(i[0]): i[1] for i in items}
        btns = [[InlineKeyboardButton(i[1][:40], callback_data=f"sel_{i[0]}")] for i in items[:40]]
        await query.message.reply_text("ðŸŽ¯ *Select Target Item:*", reply_markup=InlineKeyboardMarkup(btns))
        return SELECT_ITEM
    except Exception as e:
...
async def section_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        context.user_data['item_names'] = {str(i[0]): i[1] for i in items}
        btns = [[InlineKeyboardButton(i[1][:40], callback_data=f"sel_{i[0]}")] for i in items[:40]]
        await query.message.reply_text("ðŸŽ¯ *Select Test Series:*", reply_markup=InlineKeyboardMarkup(btns))
        return SELECT_ITEM
    except Exception as e:
