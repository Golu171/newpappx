...
async def handle_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if choice == "agg":
            ...
            # GUNDA LOGIC: Trying different search terms if one fails
            for term in ["TEST SERIES", ""]:
                p = {"search_term": term, "user_id": "-1", "screen_name": "Dashboard"}
        ...
        if not items:
            await query.message.reply_text("❌ No items found! Try another mode or check URL."); return ConversationHandler.END
...
async def item_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    api_url = context.user_data['api_url']
    status_msg = await query.edit_message_text("🕵️ *Scanning...*")
    all_tests = []
    ...
    try:
        if mode == "agg":
            ...
            for s in subj.get("data", []):
                ...
                for t in t_resp.get("test_titles", []):
                    if t.get("test_questions_url"): all_tests.append({'title': t['title'], 'link': t['test_questions_url'], 'folder': s['subject_name']})
            # If Series scan failed, try recursive as fallback
            if not all_tests:
