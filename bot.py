    ...
    try:
        ...
        for item in resp.get("data", []):
            ...
            if m_type == "TEST":
                ...
                if tid and tid != "-1":
                    # Fix: User-ID should be dynamic based on headers
...
async def get_api_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "hsscguide" in url_input and "api" not in url_input: url_input = url_input.replace("hsscguide.", "hsscguideapi.")
...
async def get_creator_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data['is_akamai']:
        kb = [[InlineKeyboardButton("ðŸš€ HSSC Special (Auto)", callback_data="type_hssc")],
...
async def handle_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"ðŸ“¡ Scanning {choice.upper()}... wait bhai.")
    ...
    try:
        ...
        if choice == "agg":
            ...
            btns = [[InlineKeyboardButton(s[1], callback_data=f"sec_{s[0]}")] for s in sections if s[1]]
            await query.message.reply_text("ðŸ“š *Select Section:*", reply_markup=InlineKeyboardMarkup(btns))
            ...
            items_raw = r.get("courses_with_folder") or r.get("data") or []
            items = [(c.get("id"), c.get("course_name") or c.get("title")) for c in items_raw]
        
        elif choice == "course":
            p = {"search_term": "TEST SERIES", "user_id": "-1"}
            r = scraper.post(f"{api_url}/get/search", headers=headers, json=p).json()
            items_raw = r.get("courses_with_folder") or r.get("data") or r.get("courses") or []
            items = [(c.get("id"), c.get("course_name") or c.get("title")) for c in items_raw]
        
        else: # Series aur HSSC logic
            r = scraper.get(f"{api_url}/get/test_series?start=-1", headers=headers).json()
            items_raw = r.get("data") or r.get("test_series") or []
            items = [(ts.get("id"), ts.get("title") or ts.get("testseries_name")) for ts in items_raw]
        ...
        await query.message.reply_text(f"âŒ Error: {e}"); return ConversationHandler.END
...
async def item_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    api_url = context.user_data['api_url']
    status_msg = await query.edit_message_text("ðŸ•µï¸ *Scanning Content (Deep Recursive)...*")
    all_tests = []
    
    try:
        # Aggregator Special Scan
        if mode == "agg":
            ...
            # Fix: HSSC Guide items are often PACKAGES, so they need Recursive Scan primarily
            ...
            # If recursive failed, try subject scan (Fallback)
            if not all_tests:
...
async def start_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "zip":
        ...
        await query.message.reply_document(document=open(zip_name, 'rb'), caption=f"ðŸš€ Extraction Done!"); os.remove(zip_name)
    else:
        for t in extracted:
            try:
                ...
                await query.message.reply_document(document=open(path, 'rb'), caption=f"ðŸ“ {t['folder']}\nâœ… {t['title']}")
