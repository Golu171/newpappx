...
async def get_creator_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🤔 *Select Extraction Mode:*", reply_markup=InlineKeyboardMarkup(kb))
    return CHOOSE_TYPE
...
async def handle_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "educourse":
        await query.edit_message_text("📡 *Fetching EduGuru Courses... Wait bhai*")
        try:
            ...
            await query.message.reply_text("🎯 *Select EduGuru Course:*", reply_markup=InlineKeyboardMarkup(btns))
            return SELECT_ITEM
        except Exception as e:
            await query.message.reply_text(f"💥 Course API Error: {str(e)}"); return ConversationHandler.END

    elif choice == "hssc":
        await query.edit_message_text("📡 *Fetching HSSC Guide Series... Auto-Fetch*")
        try:
            r = scraper.get(f"{api_url}/get/test_series?start=-1", headers=headers, timeout=20).json()
            items = [(ts["id"], ts["title"]) for ts in r.get("data", [])]
            context.user_data['item_names'] = {str(i[0]): i[1] for i in items}
            btns = [[InlineKeyboardButton(i[1][:40], callback_data=f"sel_{i[0]}")] for i in items[:40]]
            await query.message.reply_text("🎯 *Select HSSC Series:*", reply_markup=InlineKeyboardMarkup(btns))
            return SELECT_ITEM
        except Exception as e:
            await query.message.reply_text(f"💥 HSSC API Error: {str(e)}"); return ConversationHandler.END

    elif choice == "agg":
        await query.edit_message_text("📡 *Fetching Aggregator Sections...*")
        try:
            ...
            for i in range(0, len(sections), 2):
                ...
                btns.append(row)
            await query.message.reply_text("📚 *Select Exam Section:*", reply_markup=InlineKeyboardMarkup(btns))
            return SELECT_SECTION
        except Exception as e:
            await query.message.reply_text(f"💥 Aggregator Error: {str(e)}"); return ConversationHandler.END
    ...
    try:
        ...
        context.user_data['item_names'] = {str(i[0]): i[1] for i in items}
        btns = [[InlineKeyboardButton(i[1][:40], callback_data=f"sel_{i[0]}")] for i in items[:40]]
        await query.message.reply_text("🎯 *Select Target Item:*", reply_markup=InlineKeyboardMarkup(btns))
        return SELECT_ITEM
    except Exception as e:
...
async def section_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        context.user_data['item_names'] = {str(i[0]): i[1] for i in items}
        btns = [[InlineKeyboardButton(i[1][:40], callback_data=f"sel_{i[0]}")] for i in items[:40]]
        await query.message.reply_text("🎯 *Select Test Series:*", reply_markup=InlineKeyboardMarkup(btns))
        return SELECT_ITEM
    except Exception as e:
        await query.message.reply_text(f"❌ Series Error: {e}"); return ConversationHandler.END
...
async def item_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await query.edit_message_text("🕵️ *Scanning Content Topic-Wise...*")
    all_tests = []
    try:
        if mode == "agg":
    ...
    await status_msg.edit_text(f"✅ Found {len(all_tests)} tests!\nChoose Upload Method:", reply_markup=InlineKeyboardMarkup(kb))
