        ...
        for item in resp.get("data", []):
            if item.get("material_type") == "TEST":
                ...
                if tid and tid != "-1":
                    ...
                    d = scraper.get(t_url, headers=headers).json().get("data", {})
...
async def get_api_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "hsscguide" in url and "api" not in url: url = url.replace("hsscguide.", "hsscguideapi.")
    ...
    await update.message.reply_text(f"✅ API Set: `{api_url}`\n✍️ Creator Name?")
    return CREATOR_NAME
...
async def handle_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if choice == "educourse":
            ...
            r = scraper.post(f"{base_api}/get/search", headers=headers, json={"search_term": "", "user_id": "1558", "screen_name": "Dashboard"}).json()
            ...
            sections = [(item.get("exam_id") or item.get("id"), item.get("exam_name")) for item in r.get("data", []) if item.get("exam_name")]
            ...
            r = scraper.post(f"{api_url}/get/search", headers=headers, json={"search_term": "TEST SERIES", "user_id": "-1"}).json()
            ...
            r = scraper.get(f"{api_url}/get/test_series?start=-1", headers=headers).json()
            items_raw = r.get("data") or r.get("test_series") or r.get("series") or []
            items = [(ts.get("id"), ts.get("title") or ts.get("testseries_name")) for ts in items_raw]

        if not items:
            await query.message.reply_text("❌ No items found for this mode!"); return ConversationHandler.END
...
async def section_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    items = [(ts.get("id") or ts.get("testseries_id"), ts.get("title") or ts.get("testseries_name")) for ts in items_raw]
    context.user_data['item_names'] = {str(i[0]): i[1] for i in items}
    btns = [[InlineKeyboardButton(i[1][:40], callback_data=f"sel_{i[0]}")] for i in items[:40]]
    await query.edit_message_text("🎯 *Select Series:*", reply_markup=InlineKeyboardMarkup(btns))
    return SELECT_ITEM
...
async def item_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await query.edit_message_text("🕵️ *Scanning Content...*")
    all_tests = []
    ...
    try:
        ...
        elif mode == "series" or mode == "hssc" or ("hsscguide" in context.user_data['api_url']):
        ...
        else: # Course / EduCourse
            api_url = "https://hsscgurukulapi.akamai.net.in" if mode == "educourse" else context.user_data['api_url']
